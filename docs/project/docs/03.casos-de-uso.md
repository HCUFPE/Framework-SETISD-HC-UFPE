...
flowchart LR
    ...
    subgraph "Microscopia"
        ...
        UC024([Marcar Exame como Liberado])
...
- **Ator**: Recepcionista.
- **Pré-condição**: Recepcionista autenticada. Peça identificada como proveniente do SUS. Exame já registrado no AGHU manualmente pela recepcionista.
- **Fluxo principal**: Selecionar origem SUS → Buscar paciente por CNS/CPF → Caso inexistente, cadastrar paciente no sistema → Importar dados do exame do AGHU pelo código do paciente/exame → Sistema gera número de solicitação → `include` UC005 → Registrar data, hora e responsável pelo recebimento.
- **Fluxo alternativo**: Paciente já cadastrado no sistema → Recuperar dados existentes e prosseguir sem novo cadastro.
...
- **Context**: Integração com AGHU ativa (somente leitura). Schema de paciente criado no banco.
- **Action**: Implementar verificação de existência do paciente por CNS/CPF antes de criar novo registro; importar dados do exame do AGHU pelo identificador informado; gerar número de solicitação único; persistir recebimento com timestamp e usuário autenticado.
- **Result**: Paciente cadastrado ou recuperado; dados do exame importados do AGHU; número de solicitação gerado; log de recebimento criado.
...
- **Ator**: Recepcionista.
- **Pré-condição**: Recepcionista autenticada. Peça identificada como proveniente do HC. Exame já registrado no AGHU.
- **Fluxo principal**: Selecionar origem HC → Escanear código de barras da peça via pistola **ou** buscar manualmente por número de solicitação, CNS ou CPF → Sistema importa dados do paciente e exame do AGHU → `include` UC005.
- **Fluxo alternativo**: Código/dado não encontrado no AGHU → Exibir mensagem de erro → Permitir nova tentativa.
- **Pós-condição**: Dados da peça importados do AGHU; etiqueta do sistema impressa; status "Na Triagem".
- **RFs relacionados**: RF005, RF008, RF009, RF011.

#### [CARE-UC004] Consulta de Peça HC

- **Context**: Leitor HID conectado à estação da triagem. Integração com AGHU ativa (somente leitura).
...
- **Result**: Dados do paciente e exame importados do AGHU e exibidos; fluxo segue para geração de etiqueta.
...
- **RFs relacionados**: RF010, RF054, RF055.
...
- **Fluxo principal**: Escanear QR Code do frasco **ou** buscar manualmente pelo número de solicitação → Sistema exibe visão unificada com dados do paciente e exame importados do AGHU e histórico local.
...
- **RFs relacionados**: RF013, RF041, RF051, RF052.

#### [CARE-UC007] Identificação do Frasco na Macroscopia

- **Context**: Integração com AGHU ativa (somente leitura). Frasco com status "Aguardando Macroscopia".
- **Action**: Implementar leitura de QR Code via input HID e campo de busca manual; ao identificar a peça, consultar dados complementares no AGHU e montar a visão unificada com histórico local.
...
- **Fluxo principal**: Preencher descrição da macroscopia → Informar número de cassetes → Confirmar data e responsável → Sistema persiste o registro localmente.
- **Pós-condição**: Macroscopia registrada no sistema; fluxo segue para geração de cassetes.
- **RFs relacionados**: RF014.

#### [CARE-UC008] Registro da Macroscopia

- **Context**: Frasco identificado na sessão. Registro mantido exclusivamente no sistema.
- **Action**: Implementar formulário de registro da macroscopia com persistência local de descrição, data, número de cassetes e responsável; atualizar status da peça.
- **Result**: Dados persistidos localmente; status da peça atualizado.
...
- **Action**: Implementar criação de N cassetes vinculados à peça com identificadores únicos; gerar e imprimir etiqueta QR Code para cada um; confirmação por leitura HID ou seleção manual na listagem.
...
- **Action**: Implementar transição de status em lote para todos os cassetes do conjunto; persistir responsável e timestamp; atualizar fila de processamento.
...
- **Pós-condição**: Corte registrado; lâminas geradas e vinculadas hierarquicamente ao bloco; etiquetas impressas.
- **RFs relacionados**: RF023, RF024, RF027.
...
- **RFs relacionados**: RF029, RF031, RF040.
...
- **Fluxo principal**: Escanear QR Code da lâmina **ou** buscar manualmente pelo identificador → Sistema exibe visão unificada com dados do paciente, exame, histórico de processamento e dados importados do AGHU.
...
- **RFs relacionados**: RF033, RF041, RF051, RF052.

#### [CARE-UC020] Identificação da Lâmina na Microscopia

- **Context**: Integração com AGHU ativa (somente leitura). Lâmina com status "Aguardando Microscopia".
- **Action**: Implementar leitura por QR Code via input HID e campo de busca manual; ao identificar, consolidar dados locais com dados do AGHU e exibir visão unificada.
...
- **Fluxo principal**: Preencher laudo prévio → Confirmar registro → Sistema persiste o laudo localmente vinculado à lâmina e ao exame → Atualiza status para "Laudo Prévio Registrado".
- **Pós-condição**: Laudo prévio disponível para revisão do médico patologista. Registro mantido exclusivamente no sistema.
- **RFs relacionados**: RF034.

#### [CARE-UC021] Registro do Laudo Prévio

- **Context**: Lâmina identificada. Registro mantido exclusivamente no sistema, sem escrita no AGHU.
- **Action**: Implementar formulário de laudo prévio com persistência local; atualizar status da lâmina/exame após confirmação; tornar o laudo visível na fila de revisão do patologista.
- **Result**: Laudo prévio persistido localmente; exame visível na fila de revisão.
- **Evaluation**: Validar manualmente o registro e a visibilidade na fila de revisão em ambiente de homologação.
...
- **Pós-condição**: Laudo aprovado segue para marcação de liberado; laudo reprovado gera pedido de complemento.
- **RFs relacionados**: RF035.
...
- **Result**: Laudo aprovado avança para marcação de liberado; laudo reprovado dispara fluxo de solicitação de complemento.
...
- **Ator**: Médico Patologista.
- **Pré-condição**: Laudo prévio aprovado (UC022 concluído). O patologista registrou e liberou o laudo final diretamente no AGHU.
- **Fluxo principal (automático)**: Sistema detecta a liberação do laudo no AGHU via leitura de status → Marca automaticamente o exame como "Liberado" → `include` UC025.
- **Fluxo alternativo (manual)**: Integração com AGHU não permite leitura de status → Médico patologista confirma manualmente no sistema que o laudo foi liberado no AGHU → Sistema marca o exame como "Liberado" → `include` UC025.
- **Pós-condição**: Exame marcado como "Liberado" no sistema; fluxo segue para arquivamento das lâminas.
- **RFs relacionados**: RF038, RF053.

#### [CARE-UC024] Marcação de Exame como Liberado

- **Context**: Laudo prévio aprovado. O laudo final é registrado e liberado pelo patologista diretamente no AGHU, fora do sistema. Integração com AGHU é somente leitura.
- **Action**: Implementar dois caminhos: (1) consulta periódica ou por evento ao AGHU para detectar mudança de status do exame para liberado, acionando a marcação automática; (2) ação manual do patologista no sistema para confirmar a liberação, como fallback. Ambos devem registrar responsável e timestamp.
- **Result**: Exame com status "Liberado"; histórico atualizado; lâminas sinalizadas para arquivamento.
- **Evaluation**: Validar manualmente o caminho manual em ambiente de homologação; validar o caminho automático quando a integração com o AGHU estiver disponível.
...
- **Ator**: Médico Patologista.
- **Pré-condição**: Exame marcado como liberado (UC024 concluído).
...
- **RFs relacionados**: RF039, RF040.

#### [CARE-UC025] Arquivamento de Lâminas Pós-Laudo

- **Context**: Lâminas com exame liberado. Médico Patologista autenticado.
...
- **Atores**: Recepcionista, Macroscopista, Técnico de Laboratório, Residente, Médico Patologista.
- **Pré-condição**: Usuário autenticado.
- **Fluxo principal**: Escanear QR Code de qualquer subproduto (frasco, cassete, bloco ou lâmina) **ou** buscar manualmente pelo identificador ou número de solicitação → Sistema exibe visão unificada com hierarquia completa da peça, histórico de movimentação com timestamps e dados clínicos importados do AGHU.
- **Pós-condição**: Visão unificada exibida.
- **RFs relacionados**: RF041, RF051, RF052.

#### [CARE-UC026] Visão Unificada de Subproduto

- **Context**: Integração com AGHU ativa (somente leitura). Qualquer subproduto registrado no sistema.
- **Action**: Implementar endpoint de consulta que, dado um identificador de subproduto, retorne a hierarquia completa (peça → cassetes → blocos → lâminas), o histórico de movimentação com responsáveis e timestamps, e os dados clínicos do AGHU consultados; resposta em no máximo 2 segundos.
...
- **Pós-condição**: Status atual exibido com contexto da última movimentação; alerta exibido caso prazo da etapa esteja excedido.
- **RFs relacionados**: RF042, RF043.
...
- **RFs relacionados**: RF044, RF045, RF046, RF047, RF048, RF049.
