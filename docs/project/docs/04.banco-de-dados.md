...
erDiagram
    ...
    AGHU_SYNC {
        ...
        string status_importacao "Sucesso, Erro, Pendente"
    ...
    LAUDO_FINAL {
        ...
        boolean liberado
        datetime data_liberacao
        boolean liberado_manualmente "True se confirmado pelo patologista, False se detectado automaticamente via AGHU"
...
#### Tabela: AGHU_SYNC
Cache local de dados importados do AGHU. A integração é estritamente de leitura (somente consulta). Nenhum dado é enviado ao AGHU por esta tabela.

| Campo | Tipo | Constraints | Descrição |
|---|---|---|---|
| id_sync | UUID | PK | Identificador único do registro de importação |
...
| data_importacao | TIMESTAMP | NOT NULL | Data/hora da última importação do AGHU |
| status_importacao | VARCHAR(20) | CHECK IN ('Sucesso', 'Erro', 'Pendente'), NOT NULL | Status da última tentativa de importação |
| mensagem_erro | TEXT | NULLABLE | Mensagem de erro se status = 'Erro' |

**Regras de Integridade:**
- Cada exame tem no máximo um registro AGHU_SYNC ativo.
- Retry automático em caso de falha na importação.
...
#### Tabela: LAUDO_FINAL
Registro do laudo final validado e liberado pelo médico patologista. O laudo oficial permanece exclusivamente no AGHU; esta tabela armazena apenas o registro local de confirmação de liberação.
...
| liberado | BOOLEAN | DEFAULT FALSE | Indica se o exame foi marcado como liberado no sistema |
| data_liberacao | TIMESTAMP | NULLABLE | Data/hora de marcação como liberado |
| liberado_manualmente | BOOLEAN | NOT NULL | TRUE se confirmado manualmente pelo patologista; FALSE se detectado automaticamente via leitura de status do AGHU |
| numero_laudo_aghu | VARCHAR(50) | NULLABLE | Referência ao laudo no AGHU |
| data_criacao | TIMESTAMP | DEFAULT CURRENT_TIMESTAMP | Data de criação |

**Regras de Integridade:**
- Data de liberação é registrada automaticamente quando o exame é marcado como liberado.
- Número do laudo no AGHU é preenchido manualmente pelo patologista ao confirmar a liberação no AGHU, ou automaticamente via leitura de status, quando a integração permitir.
- `liberado_manualmente` deve ser sempre preenchido junto com `liberado = TRUE`.
...
### [SCHEMA] Esquema de Importação AGHU
Validação dos dados importados do AGHU (somente leitura).
...
Na Triagem 
  ...
  → Laudo Elaborado 
  → Revisão Pendente
      → Aguardando Complemento (laudo reprovado, pedido de imunohistoquímica/complementos)
          → Em Processamento (retrabalho, retorna ao fluxo de corte)
          → Aguardando Microscopia (após conclusão do complemento)
          → Em Análise
          → Laudo Elaborado
          → Revisão Pendente
  → Liberado 
...
1. **Logs Obrigatórios**
   ...
   - Importações do AGHU são registradas em `AGHU_SYNC`.
...
1. **Importação do AGHU (Somente Leitura)**
   - Dados clínicos são importados do AGHU e armazenados em `AGHU_SYNC` como JSON.
   - O sistema não exporta dados para o AGHU. Laudos finais são registrados exclusivamente no AGHU pelo patologista.
   - Em caso de falha na importação, retry automático é acionado com backoff exponencial.
